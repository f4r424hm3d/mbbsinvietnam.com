...
import { UniversityScholarship } from '../../../Api';
    ...
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden group flex flex-col h-full">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 text-white">
        <div className="flex justify-between items-start mb-3">
          <div>
            <h3 className="text-xl font-bold mb-1">University Scholarship</h3>
            <div className="flex items-center text-blue-100">
              <MapPin className="w-4 h-4 mr-1" />
              <span className="text-sm">Kyrgyzstan</span>
            </div>
          </div>
          <div className="flex flex-col items-end">
            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getTypeColor(scholarship.scholarship_type)} bg-opacity-20 text-white border border-white border-opacity-30`}>
        ...
        <div className="border-t border-blue-500 border-opacity-30 pt-3">
          <h4 className="text-lg font-semibold">{scholarship.title}</h4>
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">{formatAmount(scholarship.amount_min, scholarship.amount_max)}</p>
              {scholarship.discount_percentage && (
                <p className="text-xs text-green-600">{scholarship.discount_percentage}% off tuition</p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">{scholarship.available_seats} seats</p>
            </div>
          </div>
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">{scholarship.program}</p>
            </div>
          </div>
        </div>
        ...
        {/* Application Mode */}
        <div className="mb-4">
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${getModeColor(scholarship.application_mode)}`}>
            {scholarship.application_mode.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
          </span>
        </div>
        ...
        <button
          onClick={() => navigate(`/scholarships/${scholarship.university_id}/${scholarship.id}`)}
